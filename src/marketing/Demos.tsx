...
export function LeadCardDemo({stage='Qualified',compact=false}:{stage?:string;compact?:boolean}) {
  return <article className={'lf-lead-card '+(compact?'lf-compact':'')}><div className="lf-lead-identity"><span className="lf-avatar">JM</span><div><strong>James Mwangi</strong><small>Website enquiry · Acacia Properties</small></div><span className="lf-score">86</span></div><h4>3-bedroom apartment</h4><p>Kilimani · KSh 18M–22M</p><div className="lf-lead-bottom"><span className="lf-badge">{stage}</span><span>Sarah · next: arrange viewing</span></div></article>
}
...
export function HeroDemo() {
  ...
  return <div ref={scene.ref} className="lf-hero-scene"><div ref={wrap} className="lf-perspective" onPointerMove={pointer} onPointerLeave={reset}><DemoShell title="Acacia Properties / Lead workspace"><div className="lf-hero-demo-grid">
    <section className="lf-conversation-preview"><div className="lf-pane-title"><span className="lf-channel-dot"/>Website / WhatsApp scenario <span>10:32</span></div><div className="lf-bubble customer"><small>James Mwangi</small>I'm looking for a 3-bedroom apartment in Kilimani.</div>
  ...
  </div></DemoShell></div><div className="lf-scene-caption"><span key={scene.step}>{heroSteps[scene.step]}</span><SequenceControls step={scene.step} count={7} choose={scene.choose} replay={scene.replay} playing={scene.playing} toggle={()=>scene.setPlaying(!scene.playing)} reduced={scene.reduced}/></div><p className="lf-fine">A fictional real-estate scenario. Scripted responses; no messages are sent.</p></div>
}
...
const story = [
  ...
  ['A clear next step. A clear owner.','James becomes a qualified opportunity. Sarah receives the conversation, the requirements and a viewing to arrange.'],
]
function TransformScene({step}:{step:number}) {
  ...
  {step>=2 && <div className="lf-transform-record" key={step}>{step===2?<><blockquote>“My budget is around 20 million and I’d like to move within three months.”</blockquote><div className="lf-qualification-fields">{qualification.map(([label,value])=><div className="is-filled" key={label}><small>{label}</small><strong>{value}</strong><span>✓</span></div>)}</div></>:<><LeadCardDemo/><div className="lf-handoff">✓ Assigned to Sarah<small>Next action: arrange a viewing.</small></div><p className="lf-fine">Score 86 is illustrative, based on this demo’s completed fields and stated intent. It is not a prediction of purchase.</p></>}</div>}</DemoShell>
}
...
const answers = [
  ["I'm looking for an apartment.","Which location are you considering, what is your budget, and when would you like to move?","Collect interest, budget and timeline"],
  ['How much does this cost?',"I don’t have a verified price for that listing. Which property do you mean? I can ask an agent to confirm the current price.","Ask for clarification; never invent a price"],
...
export function CRMDemo() {
  ...
  return <DemoShell title="Leads / Team workspace"><div className="lf-demo-toolbar"><button className={filter?'is-selected':''} aria-pressed={filter} onClick={()=>setFilter(!filter)}>Qualified only {filter?'✓':'↓'}</button><button disabled={added} onClick={()=>setAdded(true)}>{added?'Sample lead added ✓':'+ Add sample lead'}</button><span>Try filtering and assigning</span></div><div className="lf-crm-table"><table><caption className="lf-sr-only">Fictional Acacia Properties leads</caption><thead><tr>{['Lead','Interest','Source','Status','Owner','Score'].map(v=><th key={v}>{v}</th>)}</tr></thead><tbody>{added&&<tr className="lf-new-row"><td>Demo enquiry</td><td>3-bedroom apartment</td><td>Website</td><td><span className="lf-badge">Qualified</span></td><td>Sarah</td><td>80</td></tr>}{records.map((l,i)=><tr key={l.name}><td><strong>{l.name}</strong></td><td>{l.interest}</td><td>{l.source}</td><td><span className="lf-badge">{l.stage}</span></td><td>{i===0?<button onClick={()=>setAssigned(!assigned)}>{assigned?'Sarah ✓':'Assign to Sarah'}</button>:l.owner}</td><td>{l.score}</td></tr>)}</tbody></table></div><div className="lf-demo-footnote">Local demo only · changes do not enter your workspace.</div></DemoShell>
}

const pipelineStages=['New','Qualified','Contacted','Meeting','Negotiation','Won']
export function PipelineDemo(){
  const s=useSequence(3,2400),stage=[0,1,3][s.step]
  return <div ref={s.ref}><DemoShell title="Pipeline / James’s next step"><div className="lf-pipeline"><div className="lf-pipeline-heads">{pipelineStages.map((name,i)=><div key={name} className={stage===i?'is-current':''}><span/> {name}</div>)}</div><div className="lf-pipeline-track"><div className="lf-moving-deal" style={{'--column':stage} as CSSProperties}><span className="lf-avatar">JM</span><strong>James Mwangi</strong><p>Kilimani apartment</p><span className="lf-badge">{pipelineStages[stage]}</span><small>Sarah · arrange viewing</small></div><div className="lf-static-deal" style={{'--column':2} as CSSProperties}><span className="lf-avatar">AN</span><strong>Aisha Njeri</strong><p>Lavington townhouse</p><span className="lf-badge">Contacted</span><small>Sarah · confirm preferences</small></div><div className="lf-static-deal" style={{'--column':5} as CSSProperties}><span className="lf-avatar">BO</span><strong>Brian Otieno</strong><p>Westlands apartment</p><span className="lf-badge">Won</span><small>David · complete handover</small></div></div></div><div className="lf-pipeline-mobile"><LeadCardDemo stage={pipelineStages[stage]}/></div></DemoShell><div className="lf-scene-caption"><span>One opportunity, a visible next step.</span><SequenceControls step={s.step} count={3} choose={s.choose} replay={s.replay} playing={s.playing} toggle={()=>s.setPlaying(!s.playing)} reduced={s.reduced}/></div></div>
}
...
const inboxExamples = {
  WhatsApp:{name:'James Mwangi',message:'Can we view the Kilimani apartment on Saturday?',answer:'I can pass your preferred date to Sarah to confirm availability.',context:'Kilimani · KSh 18–22M'},
  Website:{name:'Aisha Njeri',message:'I’m interested in the Lavington townhouse.',answer:'What budget range and move-in date should the team consider?',context:'Lavington · timeframe needed'},
  Email:{name:'Brian Otieno',message:'Could someone help me compare these listings?',answer:'I’ll share your enquiry with the team for a detailed comparison.',context:'Westlands · comparison request'},
}
