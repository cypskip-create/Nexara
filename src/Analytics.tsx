...
export function Analytics({ leads, isDemo=true }: { leads: Lead[]; isDemo?:boolean }) {
  ...
  const filtered = periodLeads.filter(l => (source === 'All' || l.source === source) && (owner === 'All' || l.owner === owner) && l.score >= minimum)
  ...
  return <div className="analytics-workbench">
    ...
    <div className="insight-controls card">
      <label>Period<select aria-label="Analytics date range" value={period} onChange={e=>{setPeriod(Number(e.target.value));setSelected(null)}}><option value={7}>Last 7 days</option><option value={30}>Last 30 days</option><option value={90}>Last 90 days</option><option value={3650}>All time</option></select></label><label>Source<select value={source} onChange={e => {setSource(e.target.value);setSelected(null)}}>{['All',...new Set(leads.map(l => l.source))].map(v => <option key={v}>{v}</option>)}</select></label>
    ...
    <p className="report-footnote">Source conversion, qualification health and date filtering use stored lead history. Response-time reporting will populate as timestamped customer messages accumulate. {isDemo?'This report contains demo data.':'This report uses your live workspace leads.'}</p>
  </div>
}
