...
export function Analytics({ leads }: { leads: Lead[] }) {
  ...
  const exportCsv = () => {
    ...
    const anchor = document.createElement('a'); anchor.href = url; anchor.download = 'leadflow-demo-analytics.csv'; anchor.click(); setTimeout(() => URL.revokeObjectURL(url),1000)
  }
  const percent = (n: number, total: number) => total ? (n / total * 100).toFixed(1) + '%' : '—'
  return <div className="analytics-workbench">
    <div className="page-heading"><div><p className="eyebrow">Pipeline intelligence · Demo workspace</p><h1>Understand your opportunities.</h1><p className="subheading">Explore the leads currently in this workspace. Every filter updates the report.</p></div><button className="btn primary" disabled={!rows.length} onClick={exportCsv}>Export filtered CSV ↓</button></div>
    ...
    <p className="report-footnote">Date comparisons and response-time reports become available once timestamped lead and message history is connected. This report contains demo data only.</p>
  </div>
}
