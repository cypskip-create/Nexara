...
export function BillingPage({organizationId,demo,canManage,notify}:{organizationId?:string;demo:boolean;canManage:boolean;notify:(message:string)=>void}){
  ...
  const action=async(plan:BillingPlan)=>{if(demo){notify('Billing preview only — no payment was created');return}if(!organizationId)return;try{await startCheckout(organizationId,plan)}catch(reason){notify(reason instanceof Error?reason.message:'Unable to start checkout')}}
