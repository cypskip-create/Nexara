...
import type { ContactRow, DatabaseLeadStage, LeadActivityRow, LeadRow } from '../../types/database'
...
export function useSupabaseWorkspaceData(organizationId:string|undefined,enabled:boolean){
  ...
  const refresh=useCallback(async()=>{
    if(!enabled||!organizationId){setRows([]);setContactRows([]);setActivityRows([]);setOwners([]);setError('');return}
    setLoading(true);setError('')
    try{
      const [nextLeads,nextContacts,nextActivity,members]=await Promise.all([listLeads(organizationId,true),listContacts(organizationId),listWorkspaceActivity(organizationId),listOrganizationMembers(organizationId)])
      setRows(nextLeads);setContactRows(nextContacts);setActivityRows(nextActivity);setOwners(members.map(member=>({id:member.userId,label:member.name})))
  ...
  return {leads,archivedLeads:mappedLeads.filter(lead=>lead.archived),contacts,owners,loading,error,refresh,addLead,updateLead,moveLead,addNote,addFollowUp,archiveLeads,addContact,updateContact,resetDemo:()=>{}}
}
